-- =====================================================
-- E-COMMERCE PLATFORM DATABASE - IDEMPOTENT BUILD SCRIPT
-- For: SQL Server 2016+ / Azure SQL Database
-- Generated: 2026-05-16
-- 
-- This script is SAFE TO RUN MULTIPLE TIMES.
-- It will drop existing objects and rebuild from scratch.
-- =====================================================
USE ecommerce_db;
GO
SET NOCOUNT ON;
...
-- =====================================================
-- STEP 0: CLEANUP - Drop constraints, indexes, tables
-- =====================================================
...
-- Drop all tables (in any order since FKs are gone)
SET @sql = '';
...
-- Drop any leftover standalone indexes (if tables were dropped, indexes are gone too,
-- but this handles edge cases where indexes might exist on system tables or views)
SET @sql = '';
SELECT @sql += 'DROP INDEX IF EXISTS [' + i.name + '] ON [' + OBJECT_NAME(i.object_id) + '];' + CHAR(13)
FROM sys.indexes i
JOIN sys.tables t ON i.object_id = t.object_id
WHERE i.name LIKE 'idx_%' AND i.type > 0 AND i.is_primary_key = 0 AND i.is_unique_constraint = 0;
EXEC sp_executesql @sql;
...
-- =====================================================
-- STEP 3: INSERT SAMPLE DATA (with existence checks)
-- =====================================================

-- Users
IF NOT EXISTS (SELECT 1 FROM users WHERE user_id = 'A1B2C3D4-E5F6-7890-1234-567890ABCDEF')
INSERT INTO users (user_id, email, password_hash, full_name, phone, role, is_active, created_at)
VALUES ('A1B2C3D4-E5F6-7890-1234-567890ABCDEF', 'ahmed@email.com', '$2b$10$92IXUNpkjO0rOQ5byMi.Ye4oKoEa3Ro9llC/.og/at2.uheWG/igi', 'Ahmed Al-Rashid', '+966501234567', 'customer', 1, '2026-05-16 10:00:00');
GO

-- Employees
IF NOT EXISTS (SELECT 1 FROM employees WHERE employee_id = 'B2C3D4E5-F6A7-8901-2345-678901BCDEF0')
INSERT INTO employees (employee_id, full_name, email, phone, department, job_title, hire_date, salary, reports_to, is_active)
VALUES ('B2C3D4E5-F6A7-8901-2345-678901BCDEF0', 'Sarah Johnson', 'sarah.j@company.com', '+966501111222', 'Logistics', 'Warehouse Manager', '2024-03-15', 85000.00, NULL, 1);
GO

-- Customers
IF NOT EXISTS (SELECT 1 FROM customers WHERE customer_id = 'A1B2C3D4-E5F6-7890-1234-567890ABCDEF')
INSERT INTO customers (customer_id, job_title, income_range, loyalty_points)
VALUES ('A1B2C3D4-E5F6-7890-1234-567890ABCDEF', 'Software Engineer', 150000.00, 250);
GO
-- ============================================
-- ADD THIS BLOCK (missing seller user)
-- ============================================
IF NOT EXISTS (SELECT 1 FROM users WHERE user_id = 'C3D4E5F6-A7B8-9012-3456-789012CDEF01')
INSERT INTO users (user_id, email, password_hash, full_name, phone, role, is_active, created_at)
VALUES ('C3D4E5F6-A7B8-9012-3456-789012CDEF01', 'contact@techworld.com', '$2b$10$92IXUNpkjO0rOQ5byMi.Ye4oKoEa3Ro9llC/.og/at2.uheWG/igi', 'TechWorld Store', '+966502222333', 'seller', 1, '2025-01-10 09:00:00');
GO
-- Sellers
IF NOT EXISTS (SELECT 1 FROM sellers WHERE seller_id = 'C3D4E5F6-A7B8-9012-3456-789012CDEF01')
...
VALUES ('C3D4E5F6-A7B8-9012-3456-789012CDEF01', 'B2C3D4E5-F6A7-8901-2345-678901BCDEF0', 'TechWorld Store', 'contact@techworld.com', '+966502222333', 'verified', 8.50, 4.7, '{"bank":"Riyad Bank","iban":"SA03...","account":"123456789"}', '2025-01-10 09:00:00');
GO

-- Premium Subscriptions
IF NOT EXISTS (SELECT 1 FROM premium_subscriptions WHERE subscription_id = 'E7F8A9B0-C1D2-3456-7890-123456012345')
INSERT INTO premium_subscriptions (subscription_id, customer_id, plan_name, start_date, end_date, monthly_fee, is_active)
VALUES ('E7F8A9B0-C1D2-3456-7890-123456012345', 'A1B2C3D4-E5F6-7890-1234-567890ABCDEF', 'Platinum', '2026-05-01', '2027-05-01', 49.99, 1);
GO

-- Addresses
IF NOT EXISTS (SELECT 1 FROM addresses WHERE address_id = 'B8C9D0E1-F2A3-4567-8901-234567123456')
...
VALUES ('B8C9D0E1-F2A3-4567-8901-234567123456', 'A1B2C3D4-E5F6-7890-1234-567890ABCDEF', 'home', 'Home', 'Ahmed Al-Rashid', '+966501234567', '123 King Fahd Road', 'Riyadh', 'Riyadh Region', 'Saudi Arabia', '11564', 1);
GO

-- Categories
IF NOT EXISTS (SELECT 1 FROM categories WHERE category_id = 'D4E5F6A7-B8C9-0123-4567-890123DEF012')
...
-- Products
IF NOT EXISTS (SELECT 1 FROM products WHERE product_id = 'F6A7B8C9-D0E1-2345-6789-012345F01234')
...
VALUES ('F6A7B8C9-D0E1-2345-6789-012345F01234', 'C3D4E5F6-A7B8-9012-3456-789012CDEF01', 'E5F6A7B8-C9D0-1234-5678-901234EF0123', 'IPH16-PRO-256', 'iPhone 16 Pro 256GB', 'iphone-16-pro-256gb', 'Latest Apple iPhone with A18 Pro chip, titanium design, and advanced camera system.', 4599.00, 150, 'Apple', 0.187, 1, 4.5, 12, '2026-01-15 08:00:00');
GO

-- Product Images
IF NOT EXISTS (SELECT 1 FROM product_images WHERE image_id = 'D0E1F2A3-B4C5-6789-0123-456789345678')
INSERT INTO product_images (image_id, product_id, image_url, alt_text, is_primary, sort_order)
VALUES ('D0E1F2A3-B4C5-6789-0123-456789345678', 'F6A7B8C9-D0E1-2345-6789-012345F01234', 'https://cdn.store.com/img/iphone16-1.jpg', 'iPhone 16 Pro Titanium Black', 1, 1);
GO

-- Product Variants
IF NOT EXISTS (SELECT 1 FROM product_variants WHERE variant_id = 'E1F2A3B4-C5D6-7890-1234-567890456789')
INSERT INTO product_variants (variant_id, product_id, variant_name, price_adjustment, stock_quantity, sku)
VALUES ('E1F2A3B4-C5D6-7890-1234-567890456789', 'F6A7B8C9-D0E1-2345-6789-012345F01234', 'Color: Titanium Black', 0, 50, 'IPH16-PRO-256-BLK');
GO

-- Warehouses
IF NOT EXISTS (SELECT 1 FROM warehouses WHERE warehouse_id = 'A7B8C9D0-E1F2-3456-7890-123456012345')
INSERT INTO warehouses (warehouse_id, manager_id, warehouse_name, location, is_active)
VALUES ('A7B8C9D0-E1F2-3456-7890-123456012345', 'B2C3D4E5-F6A7-8901-2345-678901BCDEF0', 'Riyadh Central Fulfillment', 'Riyadh Industrial City, Saudi Arabia', 1);
GO

-- Product Distributions
IF NOT EXISTS (SELECT 1 FROM product_distributions WHERE distribution_id = 'C1D2E3F4-A5B6-7890-1234-567890456789')
INSERT INTO product_distributions (distribution_id, product_id, warehouse_id, quantity_available, reserved_quantity)
VALUES ('C1D2E3F4-A5B6-7890-1234-567890456789', 'F6A7B8C9-D0E1-2345-6789-012345F01234', 'A7B8C9D0-E1F2-3456-7890-123456012345', 80, 12);
GO

-- Discounts
IF NOT EXISTS (SELECT 1 FROM discounts WHERE discount_id = 'B0C1D2E3-F4A5-6789-0123-456789345678')
...
VALUES ('B0C1D2E3-F4A5-6789-0123-456789345678', 'F6A7B8C9-D0E1-2345-6789-012345F01234', 'percentage', 15.00, '2026-05-15 00:00:00', '2026-05-20 23:59:59', NULL, 100, 0, 1);
GO

-- Coupons
IF NOT EXISTS (SELECT 1 FROM coupons WHERE code = 'SUMMER2026')
...
VALUES (NEWID(), 'SUMMER2026', 'percentage', 10.00, 100.00, 500.00, 500, 0, '2026-06-01 00:00:00', '2026-08-31 23:59:59', 1);
GO

-- Carts
IF NOT EXISTS (SELECT 1 FROM carts WHERE cart_id = 'C9D0E1F2-A3B4-5678-9012-345678234567')
INSERT INTO carts (cart_id, customer_id, total_amount, updated_at)
VALUES ('C9D0E1F2-A3B4-5678-9012-345678234567', 'A1B2C3D4-E5F6-7890-1234-567890ABCDEF', 4599.00, '2026-05-16 14:30:00');
GO

-- Cart Items
IF NOT EXISTS (SELECT 1 FROM cart_items WHERE cart_item_id = 'A9B0C1D2-E3F4-5678-9012-345678234567')
INSERT INTO cart_items (cart_item_id, cart_id, product_id, variant_id, quantity, unit_price)
VALUES ('A9B0C1D2-E3F4-5678-9012-345678234567', 'C9D0E1F2-A3B4-5678-9012-345678234567', 'F6A7B8C9-D0E1-2345-6789-012345F01234', 'E1F2A3B4-C5D6-7890-1234-567890456789', 1, 4599.00);
GO

-- Wishlists
IF NOT EXISTS (SELECT 1 FROM wishlists WHERE wishlist_id = 'F8A9B0C1-D2E3-4567-8901-234567123456')
INSERT INTO wishlists (wishlist_id, customer_id, product_id, added_at)
VALUES ('F8A9B0C1-D2E3-4567-8901-234567123456', 'A1B2C3D4-E5F6-7890-1234-567890ABCDEF', 'F6A7B8C9-D0E1-2345-6789-012345F01234', '2026-05-10 11:00:00');
GO

-- Orders
IF NOT EXISTS (SELECT 1 FROM orders WHERE order_id = 'F2A3B4C5-D6E7-8901-2345-678901567890')
...
VALUES ('F2A3B4C5-D6E7-8901-2345-678901567890', 'A1B2C3D4-E5F6-7890-1234-567890ABCDEF', 'B8C9D0E1-F2A3-4567-8901-234567123456', 'confirmed', 4599.00, 689.85, 0, 0, 3909.15, 'SAR', '{"street":"123 King Fahd Road","city":"Riyadh","country":"Saudi Arabia"}', 'TRK-001', 'Aramex', 'Please gift wrap', '2026-05-16 15:00:00');
GO

-- Order Items
IF NOT EXISTS (SELECT 1 FROM order_items WHERE order_item_id = 'A3B4C5D6-E7F8-9012-3456-789012678901')
...
VALUES ('A3B4C5D6-E7F8-9012-3456-789012678901', 'F2A3B4C5-D6E7-8901-2345-678901567890', 'F6A7B8C9-D0E1-2345-6789-012345F01234', 'E1F2A3B4-C5D6-7890-1234-567890456789', 'C3D4E5F6-A7B8-9012-3456-789012CDEF01', 1, 4599.00, 689.85, 3909.15, 332.28, 'pending', 'pending');
GO

-- Payments
IF NOT EXISTS (SELECT 1 FROM payments WHERE payment_id = 'B4C5D6E7-F8A9-0123-4567-890123789012')
...
VALUES ('B4C5D6E7-F8A9-0123-4567-890123789012', 'F2A3B4C5-D6E7-8901-2345-678901567890', 'card', 'paid', 3909.15, 'TRX-20260516-884422', '{"gateway":"Stripe","auth_code":"AUTH8822"}', '2026-05-16 15:02:00', NULL);
GO

-- Shipments
IF NOT EXISTS (SELECT 1 FROM shipments WHERE shipment_id = 'C5D6E7F8-A9B0-1234-5678-901234890123')
INSERT INTO shipments (shipment_id, order_item_id, warehouse_id, tracking_number, carrier_name, status, shipped_at, delivered_at)
VALUES ('C5D6E7F8-A9B0-1234-5678-901234890123', 'A3B4C5D6-E7F8-9012-3456-789012678901', 'A7B8C9D0-E1F2-3456-7890-123456012345', 'TRK-SA-9988776655', 'Aramex', 'processing', NULL, NULL);
GO

-- Reviews
IF NOT EXISTS (SELECT 1 FROM reviews WHERE review_id = 'D6E7F8A9-B0C1-2345-6789-012345901234')
...
VALUES ('D6E7F8A9-B0C1-2345-6789-012345901234', 'F6A7B8C9-D0E1-2345-6789-012345F01234', 'A1B2C3D4-E5F6-7890-1234-567890ABCDEF', 'F2A3B4C5-D6E7-8901-2345-678901567890', 5, 'Excellent camera and battery life! Highly recommended.', '["https://cdn.store.com/rev/img1.jpg"]', 1, 3, 'Thank you Ahmed! Enjoy your new iPhone.', '2026-05-14 09:30:00');
GO
